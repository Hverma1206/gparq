...
const CorporateEnquiry = () => {
  ...
  const [form, setForm] = useState({ company: "", name: "", email: "", phone: "", fleetSize: "", message: "" });
      ...
      <section className="pt-32 pb-16 bg-gradient-hero">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-3xl mx-auto text-center">
            ...
            <h1 className="font-display text-4xl sm:text-5xl font-bold text-foreground mb-6"><span className="text-gradient">Corporate</span> Enquiry</h1>
            <p className="text-xl text-muted-foreground">Get a custom parking solution for your organization</p>
          </motion.div>
        </div>
      </section>
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <form onSubmit={handleSubmit} className="max-w-xl mx-auto space-y-6">
            <div className="grid sm:grid-cols-2 gap-4">
              <input type="text" placeholder="Company Name" required className="w-full px-4 py-3 rounded-xl bg-card border border-border text-foreground" value={form.company} onChange={e => setForm({...form, company: e.target.value})} />
            ...
            <input type="text" placeholder="Fleet Size (approx)" className="w-full px-4 py-3 rounded-xl bg-card border border-border text-foreground" value={form.fleetSize} onChange={e => setForm({...form, fleetSize: e.target.value})} />
            ...
            <Button type="submit" variant="hero" className="w-full">Submit Enquiry<Send className="w-5 h-5 ml-2" /></Button>
          </form>
        </div>
      </section>
      <Footer />
